...
import { AppText, Button, EmptyState, IconButton, ProgressBar, SegmentedControl } from '../../components/ui';
...
export function WorkoutScreen() {
  ...
  // Coach mode guides set by set; the list is the classic logbook view.
  const [view, setView] = useState<'coach' | 'list'>('coach');
    ...
    <StackScreen>
      ...
      <View style={styles.progress}>
        <View style={styles.progressTexts}>
          ...
          <AppText variant="caption" color="textSecondary" accessibilityLabel={`Volumen: ${formatVolume(session.totalVolumeKg)} levantados`}>
            {formatVolume(session.totalVolumeKg)} levantados
          </AppText>
        </View>
      ...
      {view === 'coach' && session.exercises.length > 0 ? (
        <CoachMode session={session} onFinish={finish} />
      ) : (
